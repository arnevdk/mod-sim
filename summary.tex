...
\begin{document}
	...
	\subsubsection{Discrete eindige toestands-Markovketens}
	
	\begin{mydef}
		Als $X$ een discrete willekeurige variabele geïndexeerd met de tijd $t$ volgens $X(t)$ met domein $\mathcal{X}$, met de volgende evolutie in de tijd:
		\begin{itemize}
			\item Toestandsovergangen kunnen enkel plaatsvinden op een tijdstip $t \in \{1, 2, 3, ...\}$			
			\item Op deze op deze overgangen kan $X(t)$ verschuiven van toestand $x$ naar toestand $x'$ met kans $p(x,x') \geq 0$
		\end{itemize}
		Als $|\mathcal{X}| < \infty$ is het stochastisch proces gedefinieerd door $X(t)$ een discrete eindige toestands-Markovketen
	\end{mydef}

	Een discrete einidge toestands-Markovketen wordt volledig gekarakteriseerd door de begintoestand $X(0)$ en de kansfunctie $p(x,x')$. \\
	...
	\begin{algorithm}[!ht]
		\caption{Willekeurig pad door discrete Markovketen}
		\begin{algorithmic}[1]
			\Procedure{DiscreteMarkovRandomPath}{}
				\State $x_0 = X(0)$
				\For{$k=0,1,...$}
					\State $u \in [0,1]$ random		
					\State $x' = 0$				
					\While{$P_{x_k,x'} \leq u}$
						\State $x'=x+1$
					\EndWhile
					\State $x_{k+1} = x'$
				\EndFor
	...
	\subsubsection{Stabiele toestanden}
	...
	\textbf{Complexiteit}: $\mathcal{O}(tk^3)$\\
	\\
	...
	\subsubsection{Continue eindige toestands Markovketens}
	...
	Een continue einidge toestands-Markovketen wordt volledig gekarakteriseerd door de begintoestand $X(0)$, de vewachtte tijd-in-toestandfunctie $\mu(x)$ en de kansfunctie $p(x,x')$. \
	
	Als $p$ de overgangsmatrix is, is $P$ de cumulatieve overgangsmatrix met per rij de cdf van een toestand. Deze kan bekomen worden door de rijen van $p$ cumulatief op te tellen. Het volgende algoritme genereert volgens deze cdf een pad door een discrete eindige toestands-Markovketen.
	
	\begin{algorithm}[!ht]
		\caption{Willekeurig pad door continue Markovketen}
		\begin{algorithmic}[1]
			\Procedure{ContinuousMarkovRandomPath}{}
				\State $t=0$
				\State $x(t) = X(0)$
				\While{True}
					\State $\Delta t \sim E(\mu(x(t)))$ random
					\State $t= t+\Delta t$
					\State $u \in [0,1]$ random		
					\State $x' = 0$				
					\While{$P_{x_k,x'} \leq u}$
